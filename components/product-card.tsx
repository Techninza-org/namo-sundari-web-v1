      ...
      >
        <div className="relative">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {product.isNew && (
              <Badge className="bg-brand-olive text-white text-xs font-medium px-2 py-1 rounded">NEW</Badge>
            )}
            {product.originalPrice && (
              <Badge className="bg-brand-terracotta text-white text-xs font-medium px-2 py-1 rounded">SALE</Badge>
            )}
          </div>
          ...
          >
            <Heart
              className={`w-4 h-4 transition-colors ${isWishlisted ? "fill-brand-terracotta text-brand-terracotta" : "text-brand-charcoal"}`}
            />
          </Button>
        </div>
        ...
        <CardContent className="p-5">
          {/* Category */}
          <p className="text-xs text-brand-brown font-medium mb-1 uppercase tracking-wide">{product.category}</p>
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="text-base font-bold text-brand-charcoal">₹{product.price.toLocaleString()}</span>
            {product.originalPrice && (
